...
$(function () {
    ...
    $(document).on("input", ".product-quantity", function (e) {
        ...
        if (isNaN(quantity) || quantity <= max) {
            ...
            $(this).val(max);
            custom_notify("Product quantity should not exceed the remaining stock.", null);
        }
        order_computeCost();
    });
    ...
    $(document).on("click", "#addProduct", function (e) {
        if ($("#searchableCustomerSelected").attr("data-option-id")) {
            ...
            $("#mainOrder").append($("#fieldSource").children(".order-field").clone());
            $("#mainOrder").children().eq(itemId - 1).children(".searchable-product").attr("id", "orderProduct" + itemId);
            $("#mainOrder").children().eq(itemId - 1).children(".product-quantity").attr("id", "orderProduct" + itemId + "Quantity");
            search_searchable($("#mainOrder").children().eq(itemId - 1).children(".searchable-product").attr("id"));
            $("#mainOrder").stop().animate({ scrollTop: $("#mainOrder").prop("scrollHeight") }, 500);
        } else {
    ...
    $(document).on("click", "#confirmNewOrder", function (e) {
        ...
        if (!customerId || ($("#mainOrder").children().length === 0) || emptyFields > 0) {
            errors = true;
        }
        ...
        if (!errors) {
            ...
            for (var i = 0; i < $("#mainOrder").children().length; i++) {
                var order = {};
                var optionValues = JSON.parse($("#mainOrder").children().eq(i).find(".selected-result").attr("data-option-values"));

                var productId = optionValues["row_id"];
                var cost = optionValues["row_cost"] ? optionValues["row_cost"] : 0;
                var price = order_customPrice ? order_customerPrices[optionValues["row_identifier"].replace("-", "_").toLowerCase() + "_price"] : optionValues["row_price"];
                var quantity = $("#mainOrder").children().eq(i).find(".product-quantity").val();
                var discount = $("#mainOrder").children().eq(i).find(".item-discount").val();
            ...
            $.ajax({
                ...
                custom_playAudio("#audioName", "Assets/sounds/cha-ching SoundEffectsFactory.wav");
            }).fail(function (data) { });
        } else {
    ...
    $.ajax({
        ...
        $.contextMenu({
            ...
            callback: function (key, options) {
                var rowId = $(this).children("td").children("div").attr("data-row-id") ? $(this).children("td").children("div").attr("data-row-id") : $(this).closest("table").closest("td").siblings("td").eq(0).children("div").attr("data-row-id");
                if (key === "print") {
                    var orderId = [$(this).find("[data-row-id]").attr("data-row-id") ? $(this).find("[data-row-id]").attr("data-row-id") : $(this).closest("table").closest("tr").find("[data-row-id]").attr("data-row-id")];
                    order_print(orderId);
                }
                if (key === "edit") {
...
function order_loadPrices() {
    $.ajax({
        ...
        if (order_customPrice) {
            ...
            $("#pricesBox").empty().append(custom_quickTable("pricesListTable", data[1], false));
        }
        order_computeCost();
    }).fail(function (data) { });
}
    ...
    if (!$.fn.DataTable.isDataTable("#ordersTable")) {
        order_ordersTable = $("#ordersTable").DataTable({
            "drawCallback": function (settings) {
                var total = 0;
                var totalProfit = 0;
                Array.prototype.forEach.call(document.querySelectorAll(".order-price"), function (element, i) {
                    total += parseFloat(element.getAttribute("value"));
                    totalProfit += parseFloat(element.getAttribute("data-total-profit"));
                });
                ...
                if (order_isPermitted) {
                    ...
                    document.querySelector("#financialSummaryBox").innerHTML = custom_quickTable(tableId, tableData, false);
                }
...
function order_computeCost() {
    ...
    if (customerId) {
        $("#mainOrder .order-field").each(function () {
            var product = $(this).find(".selected-result").attr("data-option-identifier") ? $(this).find(".selected-result").attr("data-option-identifier") + (order_customPrice ? "_price" : "") : null;
            var quantity = $(this).find(".product-quantity").val();
            var discount = $(this).find(".item-discount").val();
            if (product) {
                var optionValues = JSON.parse($(this).find(".selected-result").attr("data-option-values"));
                if (quantity) {
                    var productPrice;
                    if (order_customPrice) {
                        product = product.replace("-", "_").toLowerCase();
                        productPrice = order_customerPrices[product] ? parseFloat(order_customerPrices[product]) : 0;
                    } else {
                        productPrice = optionValues["row_price"] ? optionValues["row_price"] : 0;
                    }
                    order_totalOrderCost += (productPrice * quantity) - (discount ? discount : 0);
                }
