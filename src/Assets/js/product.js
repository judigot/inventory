...
$(function () {
  $.contextMenu({
    ...
    callback: function (key, options) {
      var rowId = $(this).children("span").attr("data-row-id")
      ...
      if (key === "delete") {
        ...
        if (
          confirm("Are you sure you want to delete this " + tableType + "?")
        ) {
...
$(function () {
  ...
  $(document).on("click", "#confirmNewProduct", function (e) {
    if ($("#newProductName").val() && $("#newProductInitStock").val()) {
      ...
      newProductValues["productPrice"] = !order_customPrice
        ? $("#newProductPrice").val()
        : false;
...
function product_loadCategories() {
  ...
  return `<select id='${editor}' class="category-editor" row='1'>${categories.join(
    ""
  )}</select>`;
}
