...
function signup_confirmUser() {
  ...
  if (
    ...
    if ($("#password").val() === $("#confirmPassword").val()) {
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (data[0] === 0) {
            ...
            document.title = "Email already taken - Inventory";
            $("#login-result").text(
              "The email address that you entered has already been used."
            );
            $("#first-name").text("Hello, " + data[0] + "!");
            $("input").removeAttr("disabled");
            $(".btn").show();
            signup_titleInterval = setInterval(function () {
              if (document.title !== "Email already taken - Inventory") {
                document.title = "Email already taken - Inventory";
              } else {
                document.title = "Sign up for Inventory - Inventory";
              }
            }, 1000);
      ...
      $("#login-result").text(
        "The passwords that you've entered do not match."
      );
      document.title = "Passwords do not match - Inventory";
      signup_titleInterval = setInterval(function () {
        if (document.title !== "Passwords do not match - Inventory") {
          document.title = "Passwords do not match - Inventory";
        } else {
          document.title = "Sign up for Inventory - Inventory";
        }
